...
func main() {
    ...
    owner := "wenjielee11" // Replace with your GitHub username
    repo := "github-bot"  // Replace with your GitHub repository

    // Create dummy branches and commits
    createDummyBranchesAndCommits(ctx, client, owner, repo)

    // Create dummy issues and add labels with priority and status prefixes
    createDummyIssuesAndAddLabels(ctx, client, owner, repo)
...
func createDummyBranchesAndCommits(ctx context.Context, client *github.Client, owner, repo string) {
    ...
    for i := 1; i <= 3; i++ {
        ...
        // Create a dummy secrets file in the first branch
        if i == 1 {
            secretFilePath := "dummy.txt"
            secretFileContent := "random_value= id_sde1834a96p329"
            createDummyCommit(ctx, client, owner, repo, branchName, secretFilePath, secretFileContent, "Add dummy secrets file")
        }
    }
}
...
func createDummyIssuesAndAddLabels(ctx context.Context, client *github.Client, owner, repo string) {
    for i := 1; i <= 5; i++ {
        ...
        if err != nil {
            ...
            labels := []string{fmt.Sprintf("priority: %d", i), "status: open"}
            AddLabels(ctx, client, owner, repo, issue.GetNumber(), labels)
        }
    }
}
